...
- **Decision-time search (engine path only):** `cd agent && uv run python search_eval.py --ckpt models_clauses/pg_best.pt --battles 100 --rollouts 3 --clauses` — 1-ply lookahead (clone+reseed the engine, value-head leaf, heuristic opponent model) plays ~**+19%** over the raw greedy policy vs the staller. The lever past the self-play ceiling: the agent computes a better move than its policy without a stronger opponent. `--sweep` maps rollouts/leaf headroom. **Diagnose first** (`diagnose_policy.py`): all the raw-pilot weaknesses I hypothesized (switch-loop, can't-play-defense) were refuted by measurement — the net is a competent *average* player; its holes only a strong human triggers.
- **Expert iteration (`expert_iter.py`):** `cd agent && uv run python expert_iter.py --init models_clauses/pg_best.pt --rounds 5 --games 60 --gen-teams --clauses --out ei` — self-play where both sides move by search, distil the search action (policy CE) + outcome (value MSE) back into the net, repeat. Folds search's gain into the policy so it's free at inference; the value head becomes win/loss-calibrated (a better search leaf) and the clause-fail feature is learned here. Watch `raw greedy vs smart` climb across rounds.
